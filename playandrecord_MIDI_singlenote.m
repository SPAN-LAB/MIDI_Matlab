...
%% Editing the template MIDI message
tmp_msgs = msgs;

    tmp_msgs(1).Note  = sel_note_num;
    tmp_msgs(2).Note  = sel_note_num;

    tmp_msgs(1).Timestamp  = 0.;
    tmp_msgs(2).Timestamp  = record_time;

%%
record(recorder, 3*record_time)
pause(1*record_time)
...
output_note = input_note;
output_note = strrep(output_note, '#', 'sharp');
outname = fullfile(outdir, [output_note, '.wav']);
